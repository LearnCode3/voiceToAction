...
{
    // ------------------------------------------------------------------
    // USERS — list, activate, ban, delete
    // ------------------------------------------------------------------
    ...
    public function banUser(User $user)
    {
        ...
        if ($user->isSuperAdmin()) {
            return back()->with('error', 'Cannot ban a super administrator.');
        }
    ...
    public function deleteUser(User $user)
    {
        ...
        // Nullify their reports (don't delete reports)
    ...
    // ------------------------------------------------------------------
    // STAFF — list, create, assign office, delete
    // ------------------------------------------------------------------

    public function staff(Request $request)
    {
        $staff   = User::where('role', 'office_staff')->with('office')->latest()->paginate(20);
    ...
    public function createStaff(Request $request)
    {
        $data = $request->validate([
            ...
            'office_id' => ['required', 'string', 'exists:offices,slug'],
        ]);
        ...
        return redirect()->route('admin.staff.index')
            ->with('success', "Staff member \"{$data['name']}\" created and assigned to office.");
    }

    public function reassignStaff(Request $request, User $user)
    {
        if (!$user->isOfficeStaff()) {
            return back()->with('error', 'User is not office staff.');
        }
        $data = $request->validate([
            'office_id' => ['required', 'string', 'exists:offices,slug'],
        ]);
        $user->update(['office_id' => $data['office_id']]);
        return back()->with('success', "Staff member \"{$user->name}\" reassigned successfully.");
    }

    public function activateStaff(User $user)
    {
        $user->update(['is_active' => true]);
        return back()->with('success', "Staff member \"{$user->name}\" activated.");
    }

    public function banStaff(User $user)
    {
        $user->update(['is_active' => false]);
        return back()->with('success', "Staff member \"{$user->name}\" banned.");
    }
    ...
    public function deleteStaff(User $user)
    {
        ...
        $name = $user->name;
        $user->delete();
        return back()->with('success', "Staff member \"{$name}\" deleted.");
    }

    // ------------------------------------------------------------------
    // OFFICE CRUD (moved from OfficeController for clarity)
    // keep separate so OfficeController stays focused on panel/reports
    // ------------------------------------------------------------------
    ...
    public function officesStore(Request $request)
    {
        ...
        $base = $data['slug'];
        $i    = 1;
        while (Office::where('slug', $data['slug'])->exists()) {
            $data['slug'] = $base . '-' . $i++;
        }

        Office::create($data);
        return redirect()->route('admin.offices.index')
            ->with('success', "Office \"{$data['name']}\" created successfully.");
    }
    ...
    public function officesUpdate(Request $request, Office $office)
    {
        ...
        if ($office->name !== $data['name']) {
            Report::where('office_id', $office->slug)->update(['office_name' => $data['name']]);
        }
    ...
    {
        if ($office->reports()->exists()) {
            return back()->with('error',
                "Cannot delete \"{$office->name}\" — it has {$office->reports()->count()} report(s). Deactivate it instead.");
        }
